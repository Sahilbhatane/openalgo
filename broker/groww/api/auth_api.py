...
def get_access_token_via_totp(api_key, api_secret):
    ...
    try:
        ...
        # Exact payload format from SDK - includes 'type' field
        # The 'type' field indicates the authentication method
        payload = {"totp": totp, "type": "totp"}  # Required field indicating auth type

        # Exact endpoint from SDK config
        endpoint = "https://api.groww.in/v1/token/api/access"

        logger.debug(f"Groww auth request to {endpoint} with type=totp")
